...
# MAGIC %md
# MAGIC # Japan (JP) Deep Dive — Payment Orchestration Experiment
# MAGIC
# MAGIC The main experiment analysis identified **JP (Japan)** as the country with the
# MAGIC largest success-rate regression in variant B (in-house routing). This notebook
...
# MAGIC
# MAGIC **Sections:**
# MAGIC 1. Data extraction — JP-only attempts
# MAGIC 2. High-level JP comparison — volume, SR, 3DS rates
# MAGIC 3. Error & response code analysis — what fails differently in B?
# MAGIC 4. 3DS deep dive for JP — challenge rate, challenge success, flow
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 1 — Data Extraction (JP only)
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 2 — High-Level JP Comparison

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2.1 Overall JP metrics by variant
...
print("=" * 90)
print(f"JP SUMMARY BY VARIANT")
print("=" * 90)
for _, r in jp_summary.iterrows():
    print(f"\n  {r['group_name']}:")
...
if len(jp_summary) == 2:
    ra, rb = jp_summary.iloc[0], jp_summary.iloc[1]
    print(f"\n{'─' * 90}")
    print("STATISTICAL TESTS (JP):")
...
for i, (col, title) in enumerate(metrics):
    colors = [A_COLOR, B_COLOR][:len(jp_summary)]
    bars = axes[i].bar(jp_summary["group_name"], jp_summary[col], color=colors)
    axes[i].set_title(title, fontweight="bold", fontsize=11)
    axes[i].yaxis.set_major_formatter(mticker.FuncFormatter(pct_fmt))
    for bar, val in zip(bars, jp_summary[col]):
...
fig.suptitle(f"JP — Key Metrics by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ## 3.1 Error code distribution — A vs B (JP failed attempts)

# COMMAND ----------

jp_errors = (
    df_jp.filter(F.col("is_successful") == 0)
    .groupBy("group_name", "error_code")
    .agg(F.count("*").alias("cnt"))
    .toPandas()
)

total_errors_by_grp = jp_errors.groupby("group_name")["cnt"].sum()
top_errors = jp_errors.groupby("error_code")["cnt"].sum().nlargest(15).index
plot_err = jp_errors[jp_errors["error_code"].isin(top_errors)].copy()
plot_err["pct"] = plot_err.apply(
    lambda r: r["cnt"] / total_errors_by_grp.get(r["group_name"], 1), axis=1,
)

print("JP Error Code Distribution (share of failed attempts):")
...
ax.set_title("JP — Top Error Codes (share of failed attempts)", fontsize=14, fontweight="bold")
...
jp_resp = (
    df_jp.groupBy("group_name", "response_code")
    .agg(F.count("*").alias("cnt"))
    .toPandas()
)

total_by_grp = jp_resp.groupby("group_name")["cnt"].sum()
top_resp = jp_resp.groupby("response_code")["cnt"].sum().nlargest(15).index
plot_resp = jp_resp[jp_resp["response_code"].isin(top_resp)].copy()
...
ax.set_title("JP — Top Response Codes (share of all attempts)", fontsize=14, fontweight="bold")
ax.xaxis.set_major_formatter(mticker.FuncFormatter(pct_fmt))
ax.legend(fontsize=10)
fig.tight_layout()
plt.show()
...
error_pivot = jp_errors.pivot_table(
...
status_dist = (
    df_jp.groupBy("group_name", "payment_attempt_status")
    .agg(F.count("*").alias("cnt"))
    .toPandas()
)
...
ax.set_title("JP — Payment Attempt Status Distribution", fontsize=14, fontweight="bold")
ax.xaxis.set_major_formatter(mticker.FuncFormatter(pct_fmt))
ax.legend(fontsize=10)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 4 — 3DS Deep Dive for JP
...
total_by_grp_path = jp_3ds_outcomes.groupby(["group_name", "three_ds_path"])["cnt"].sum().to_dict()
jp_3ds_outcomes["pct_of_path"] = jp_3ds_outcomes.apply(
    lambda r: r["cnt"] / total_by_grp_path.get((r["group_name"], r["three_ds_path"]), 1), axis=1,
)

for path in ["Challenged", "Not Challenged"]:
    subset = jp_3ds_outcomes[jp_3ds_outcomes["three_ds_path"] == path]
    if subset.empty:
        continue
    print(f"\n{'=' * 70}")
    print(f"JP — {path} attempts — status breakdown:")
    print(f"{'=' * 70}")
...
total_3ds_fails = jp_3ds_errors.groupby("group_name")["cnt"].sum()

print("JP — Failed 3DS-challenged attempts — error breakdown:")
...
combined_key = jp_3ds_errors.groupby(["error_code", "response_code", "payment_attempt_status"])["cnt"].sum()
for (ec, rc, st), _ in combined_key.sort_values(ascending=False).head(20).items():
    print(f"{str(ec):<35} {str(rc):<15} {str(st):<20}", end="")
    for g in groups:
        row = jp_3ds_errors[
            (jp_3ds_errors["error_code"] == ec) &
            (jp_3ds_errors["response_code"] == rc) &
            (jp_3ds_errors["payment_attempt_status"] == st) &
            (jp_3ds_errors["group_name"] == g)
        ]
        if not row.empty:
...
# MAGIC %md
# MAGIC ## 4.3 Fraud pre-auth result for JP
...
total_by_grp_fraud = jp_fraud.groupby("group_name")["cnt"].sum()
jp_fraud["pct"] = jp_fraud.apply(
    lambda r: r["cnt"] / total_by_grp_fraud.get(r["group_name"], 1), axis=1,
)
jp_fraud["sr"] = jp_fraud["successes"] / jp_fraud["cnt"]

fig, axes = plt.subplots(1, 2, figsize=(18, 6))
fraud_results = jp_fraud.groupby("fraud_pre_auth_result")["cnt"].sum().sort_values(ascending=True).index.tolist()
y = np.arange(len(fraud_results))
h = 0.35

for ax_idx, (col, title) in enumerate([("pct", "Share of Attempts"), ("sr", "Success Rate")]):
    for i, grp in enumerate(groups):
        grp_data = jp_fraud[jp_fraud["group_name"] == grp].set_index("fraud_pre_auth_result").reindex(fraud_results)
...
fig.suptitle("JP — Fraud Pre-Auth Result by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ## 5.1 Success rate by payment processor (JP)
...
fig, axes = plt.subplots(1, 2, figsize=(18, 6))
proc_list = jp_proc.groupby("payment_processor")["attempts"].sum().sort_values(ascending=True).index.tolist()
y = np.arange(len(proc_list))
h = 0.35

for ax_idx, (col, title) in enumerate([("sr", "Attempt SR"), ("challenge_rate", "3DS Challenge Rate")]):
    for i, grp in enumerate(groups):
        grp_data = jp_proc[jp_proc["group_name"] == grp].set_index("payment_processor").reindex(proc_list)
        axes[ax_idx].barh(y + i * h, grp_data[col].fillna(0), h,
                          label=grp if ax_idx == 0 else "", color=A_COLOR if i == 0 else B_COLOR)
...
axes[0].legend(fontsize=10)
fig.suptitle("JP — Processor Performance by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ## 5.2 Success rate by payment method variant (JP)
...
print("JP — Payment Method Variant Breakdown:")
print(f"{'PMV':<20} {'Group':<20} {'Attempts':>10} {'SR':>8} {'ChRate':>8} {'ChSR':>8}")
print("-" * 80)
for pmv in jp_pmv.groupby("payment_method_variant")["attempts"].sum().sort_values(ascending=False).index:
    for g in groups:
        row = jp_pmv[(jp_pmv["payment_method_variant"] == pmv) & (jp_pmv["group_name"] == g)]
        if not row.empty:
            r = row.iloc[0]
...
pmv_list = jp_pmv.groupby("payment_method_variant")["attempts"].sum().nlargest(6).sort_values(ascending=True).index.tolist()
...
for ax_idx, (col, title) in enumerate([("sr", "Attempt SR"), ("challenge_rate", "3DS Challenge Rate"), ("challenge_sr", "SR When Challenged")]):
    for i, grp in enumerate(groups):
        grp_data = jp_pmv[jp_pmv["group_name"] == grp].set_index("payment_method_variant").reindex(pmv_list)
        axes[ax_idx].barh(y + i * h, grp_data[col].fillna(0), h,
                          label=grp if ax_idx == 0 else "", color=A_COLOR if i == 0 else B_COLOR)
        for j, (val, vol) in enumerate(zip(grp_data[col].fillna(0), grp_data["attempts"].fillna(0))):
            if vol > 0:
...
fig.suptitle("JP — Payment Method Variant × Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ## 5.3 Processor × 3DS cross-tabulation for JP
...
print("JP — Processor × 3DS Path × Variant:")
print(f"{'Processor':<20} {'Path':<10} {'Group':<20} {'Attempts':>10} {'Successes':>10} {'SR':>8}")
print("-" * 85)
for proc in jp_proc_3ds["payment_processor"].unique():
    for path in ["No 3DS", "3DS"]:
        for g in groups:
            row = jp_proc_3ds[
                (jp_proc_3ds["payment_processor"] == proc) &
                (jp_proc_3ds["path"] == path) &
                (jp_proc_3ds["group_name"] == g)
            ]
            if not row.empty:
                r = row.iloc[0]
...
# MAGIC %md
# MAGIC ## 6.1 Daily attempt SR for JP by variant
...
for ax_idx, (col, title) in enumerate([
    ...
    for i, (grp, grp_data) in enumerate(jp_daily.groupby("group_name")):
    ...
    ax.set_title(f"JP — {title}", fontsize=12, fontweight="bold")
    if col != "attempts":
...
# MAGIC attempts in variant B (JP) to enable direct lookup in Primer.

# COMMAND ----------

# MAGIC %md
# MAGIC ## 7.1 Failed B-variant 3DS-challenged attempts (JP) — payment references
...
jp_b_3ds_failed_count = jp_b_3ds_failed.count()
print(f"Total failed 3DS-challenged attempts in {g_b} (JP): {jp_b_3ds_failed_count:,}")
print(f"\nShowing up to 200 payment references for Primer investigation:\n")

display(jp_b_3ds_failed.limit(200))

# COMMAND ----------

# MAGIC %md
# MAGIC ## 7.2 Failed B-variant non-3DS attempts (JP) — payment references
...
jp_b_non3ds_failed_count = jp_b_non3ds_failed.count()
print(f"Total failed non-3DS attempts in {g_b} (JP): {jp_b_non3ds_failed_count:,}")
print(f"\nShowing up to 200 payment references for Primer investigation:\n")

display(jp_b_non3ds_failed.limit(200))

# COMMAND ----------

# MAGIC %md
# MAGIC ## 7.3 Payment reference lists grouped by error code (B variant, JP)
# MAGIC
...
print("JP B-variant failed attempts — sample payment_provider_references by error code:")
print("=" * 100)
for _, r in jp_b_failed_refs.head(10).iterrows():
    refs = r["refs"][:20]
...
if g_a is not None and g_b is not None:
    top_b_errors = (
        jp_errors[jp_errors["group_name"] == g_b]
...
# MAGIC
# MAGIC Full list of `payment_provider_reference` for B-variant JP failures,
# MAGIC saved as a temp view for easy export or further querying.

# COMMAND ----------

jp_b_all_failed_refs = (
    df_jp
    .filter(F.col("group_name") == g_b)
    .filter(F.col("is_successful") == 0)
    .select(
...
jp_b_all_failed_refs.createOrReplaceTempView("jp_b_failed_references")
ref_count = jp_b_all_failed_refs.count()

print(f"Created temp view: jp_b_failed_references")
print(f"Total rows: {ref_count:,}")
print(f"\nYou can now query this view directly:")
print(f"  SELECT * FROM jp_b_failed_references WHERE error_code = '<code>' LIMIT 50")
print(f"  SELECT DISTINCT payment_provider_reference FROM jp_b_failed_references WHERE challenged = true")

display(jp_b_all_failed_refs.limit(50))
...
# MAGIC |---|---|
# MAGIC | 2.1 | How large is the SR gap for JP specifically? Is the 3DS challenge rate different? |
...
# MAGIC | 6.1 | Is the JP drop consistent across all days or concentrated in a specific period? |
# MAGIC | 7.1–7.4 | Use the payment references to look up specific transactions in Primer |
# MAGIC | 7.5 | Query `jp_b_failed_references` for ad-hoc investigation |

# COMMAND ----------

# MAGIC %md
# MAGIC ---
# MAGIC *End of JP deep-dive analysis*
