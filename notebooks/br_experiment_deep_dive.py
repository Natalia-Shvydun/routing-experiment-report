...
# MAGIC %md
# MAGIC # Brazil (BR) Deep Dive — Payment Orchestration Experiment
# MAGIC
# MAGIC The experiment (restarted 2026-04-02) shows **BR (Brazil)** performing
...
# MAGIC
# MAGIC **Sections:**
# MAGIC 1. Data extraction — BR-only attempts (with funnel columns)
# MAGIC 2. High-level BR comparison — volume, SR, 3DS rates
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 1 — Data Extraction (BR only, with funnel columns)
...
total = df.count()
print(f"Total BR payment attempts: {total:,}")
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 2 — High-Level BR Comparison

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2.1 Overall BR metrics by variant
...
fig.suptitle(f"BR — Key Metrics by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
print("BR — Processor Distribution by Variant (all system attempts)")
...
for ax_idx, (col, title) in enumerate([("pct", "Share of BR Attempts"), ("attempts", "Volume")]):
...
fig.suptitle("BR — Processor Distribution by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
if len(groups) == 2:
    ...
    print("BR — Processor share z-tests (A vs B):")
...
print("BR — Success Rate by Processor × Variant:")
...
axes[0].legend(fontsize=10)
fig.suptitle("BR — Processor Performance by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
print("BR — Payment Funnel by Processor × Variant")
...
print("BR — Customer-Attempt-Level Processor Distribution (system_attempt_rank = 1)")
print(f"{'Processor':<25}", end="")
for g in groups:
...
print("BR — Attempt Type Distribution")
...
fig.suptitle("BR — System Attempt Rank Analysis", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
print("BR — First Attempt (rank 1/1) Processor Distribution")
print(f"{'Processor':<25}", end="")
for g in groups:
...
axes[0].legend(fontsize=10)
fig.suptitle("BR — First Attempt Routing (customer_rank=1, system_rank=1)",
...
for pmv in top_pmvs:
    print(f"\n{'=' * 90}")
    print(f"Payment Method Variant: {pmv} — First Attempt Routing (BR)")
    print(f"{'=' * 90}")
...
# MAGIC %md
# MAGIC ## 6.1 Error code distribution — A vs B (BR failed attempts)
...
ax.set_title("BR — Top Error Codes (share of failed attempts)", fontsize=14, fontweight="bold")
...
for proc in top_procs:
    print(f"\n{'=' * 100}")
    print(f"{proc} — Failed Attempts Error Breakdown (BR)")
    print(f"{'=' * 100}")
...
ax.set_title("BR — Payment Attempt Status Distribution", fontsize=14, fontweight="bold")
ax.xaxis.set_major_formatter(mticker.FuncFormatter(pct_fmt))
ax.legend(fontsize=10)
fig.tight_layout()
plt.show()
...
# MAGIC %md
# MAGIC ---
# MAGIC # SECTION 7 — 3DS Deep Dive for BR
...
for path in ["Challenged", "Not Challenged"]:
    ...
    if subset.empty:
        continue
    print(f"\n{'=' * 70}")
    print(f"BR — {path} attempts — status breakdown:")
    print(f"{'=' * 70}")
...
print("BR — Failed 3DS-challenged attempts — error breakdown:")
...
print("BR — 3DS by Processor × Variant")
...
axes[0].legend(fontsize=10)
fig.suptitle("BR — Fraud Pre-Auth Result by Variant", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
plt.show()
...
print("BR — Payment Method Variant Breakdown:")
...
print("BR — Processor × 3DS Path × Variant:")
...
for ax_idx, (col, title) in enumerate([
    ...
    ax.set_title(f"BR — {title}", fontsize=12, fontweight="bold")
    if col != "attempts":
...
for grp in groups:
    ...
    ax.set_title(f"{grp} — Daily Processor Share (BR)", fontsize=13, fontweight="bold")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(pct_fmt))
...
for ax_idx, proc in enumerate(top_procs_for_daily):
    ...
    for i, (grp, grp_data) in enumerate(proc_daily.groupby("group_name")):
        ...
        axes[ax_idx].plot(grp_data["attempt_date"], grp_data["sr"], color=color,
                          linewidth=1.5, label=grp, marker=".", markersize=4)
    axes[ax_idx].set_title(f"{proc} — Daily SR (BR)", fontsize=12, fontweight="bold")
...
if g_b is not None:
    ...
    print(f"Total failed 3DS-challenged attempts in {g_b} (BR): {cnt:,}")
...
if g_b is not None:
    ...
    print(f"Total failed non-3DS attempts in {g_b} (BR): {cnt:,}")
    print(f"\nShowing up to 200 payment references for Primer investigation:\n")
...
if g_b is not None:
    ...
    print("BR B-variant failed attempts — sample payment_provider_references by error code:")
    print("=" * 100)
...
if g_b is not None:
    ...
    b_all_failed_refs.createOrReplaceTempView("br_b_failed_references")
    ref_count = b_all_failed_refs.count()

    print(f"Created temp view: br_b_failed_references")
    print(f"Total rows: {ref_count:,}")
    print(f"\nYou can now query this view directly:")
    print(f"  SELECT * FROM br_b_failed_references WHERE error_code = '<code>' LIMIT 50")
    print(f"  SELECT DISTINCT payment_provider_reference FROM br_b_failed_references WHERE challenged = true")
...
# MAGIC |---|---|
# MAGIC | 2.1 | How large is the SR gap for BR? Is the 3DS challenge rate different? |
...
# MAGIC %md
# MAGIC ---
# MAGIC *End of BR deep-dive analysis*
