...
# MAGIC %md
# MAGIC # JP Deep Dive — Routing vs Config Effect on SR
# MAGIC
...
# MAGIC
# MAGIC **Routing dimensions:** country (JP fixed here) × JPM/non-JPM currency × card scheme
...
WHERE p.payment_flow IN ('pay_now', 'rnpl_pay_early')
  ...
  AND p.bin_issuer_country_code = 'JP'
...
total = df.count()
print(f"JP customer attempts (from {PAYMENT_START}): {total:,}")
...
print(f"\n{'=' * 60}")
print(f"Overall SR — JP customer attempts")
print(f"{'=' * 60}")
...
df.createOrReplaceTempView("jp_routing_exp")
...
ax.set_ylabel("SR")
ax.set_title("Daily SR — JP Customer Attempts")
ax.legend()
...
# MAGIC %md
# MAGIC ---
# MAGIC *End of JP routing vs config deep dive*
