...
# MAGIC %md
# MAGIC # Incident Impact Analysis — Conversion Approach (Total Bookings)
# MAGIC
# MAGIC Same methodology as the conversion approach but on **total bookings**
# MAGIC (active + cancelled) — no cancellation adjustment applied.
# MAGIC
# MAGIC **Logic:**
...
# MAGIC 4. Multiply the conversion drop × expected ADP sessions → lost bookings (total)
# MAGIC 5. Apply avg GMV per total booking to get lost GMV
...
# MAGIC %md
# MAGIC # 1b. Load booking data (for avg GMV per total booking)
...
# MAGIC %md
# MAGIC ---
# MAGIC # 5. Avg GMV per total booking (for translation)
...
print(f"Avg GMV per total booking (all statuses, Mar baseline): {avg_gmv_per_booking:,.2f}")

# COMMAND ----------

# MAGIC %md
# MAGIC ---
# MAGIC # 6. Summary: incident impact (total, no cancellation adjustment)
...
lost_gmv_total = total_lost_sessions_booked * avg_gmv_per_booking

print("=" * 70)
print("INCIDENT IMPACT — CONVERSION APPROACH (TOTAL, NO CANCELLATION ADJ)")
...
print("-" * 70)
print(f"  Expected booked sessions (total): {total_expected_booked:,.0f}")
print(f"  Actual booked sessions:           {total_actual_booked:,.0f}")
print(f"  Lost booked sessions (total):     {total_lost_sessions_booked:,.0f}  ({total_lost_sessions_booked/total_expected_booked:+.1%})")
print("-" * 70)
print(f"  Avg GMV per total booking:        {avg_gmv_per_booking:,.2f}")
print(f"  Lost GMV (total):                 {lost_gmv_total:,.2f}")
print("=" * 70)

# COMMAND ----------

# MAGIC %md
# MAGIC ### 6.1 Daily breakdown (weekend vs Monday)
...
daily = (
    ...
    .agg(
        hours=("session_hour", "count"),
        actual_adp=("actual_adp_views", "sum"),
        expected_adp=("expected_adp_views", "sum"),
        actual_booked=("actual_booked", "sum"),
        expected_booked=("expected_booked", "sum"),
        lost_bookings=("lost_bookings", "sum"),
    )
    .reset_index()
)
daily["weekday"] = daily["session_date"].dt.weekday
daily["day_label"] = daily["session_date"].dt.strftime("%a %m/%d")
daily["actual_conv"] = daily["actual_booked"] / daily["actual_adp"]
daily["expected_conv"] = daily["expected_booked"] / daily["expected_adp"]
daily["conv_drop_pct"] = (daily["actual_conv"] - daily["expected_conv"]) / daily["expected_conv"] * 100
daily["lost_gmv"] = daily["lost_bookings"] * avg_gmv_per_booking
daily["share_of_lost_bookings"] = daily["lost_bookings"] / daily["lost_bookings"].sum() * 100

print("Daily breakdown:")
print(f"{'Day':<14s} {'Hours':>5s}  {'Actual':>8s}  {'Expected':>8s}  {'Lost bkgs':>9s}  {'Share%':>6s}  {'Conv Δ%':>8s}  {'Lost GMV':>14s}")
print("-" * 85)
for _, r in daily.sort_values("session_date").iterrows():
    print(f"{r['day_label']:<14s} {r['hours']:5.0f}  {r['actual_booked']:8,.0f}  {r['expected_booked']:8,.0f}  "
          f"{r['lost_bookings']:9,.0f}  {r['share_of_lost_bookings']:5.1f}%  {r['conv_drop_pct']:+7.1f}%  {r['lost_gmv']:14,.2f}")
print("-" * 85)
print(f"{'TOTAL':<14s} {daily['hours'].sum():5.0f}  {daily['actual_booked'].sum():8,.0f}  {daily['expected_booked'].sum():8,.0f}  "
      f"{daily['lost_bookings'].sum():9,.0f}  {100.0:5.1f}%  "
      f"{(daily['actual_booked'].sum()/daily['actual_adp'].sum() - daily['expected_booked'].sum()/daily['expected_adp'].sum()) / (daily['expected_booked'].sum()/daily['expected_adp'].sum()) * 100:+7.1f}%  "
      f"{daily['lost_gmv'].sum():14,.2f}")

display(spark.createDataFrame(
    daily.sort_values("session_date")
    [["day_label", "hours", "actual_adp", "expected_adp", "actual_booked",
      "expected_booked", "lost_bookings", "share_of_lost_bookings",
      "actual_conv", "expected_conv", "conv_drop_pct", "lost_gmv"]]
))
...
# MAGIC %md
# MAGIC ---
# MAGIC # 7. Hourly conversion pattern (chart-ready)
...
# MAGIC %md
# MAGIC ---
# MAGIC # 8. Cross-check: conversion rate previous week vs incident
...
# MAGIC %md
# MAGIC ---
# MAGIC # 9. Traffic validation: were ADP views actually unaffected?
